...
def knn_classifier_test():
    # 訓練集合資料用來當作測試的比例
    test_ratio = 0.05
    dating_data_matrix, dating_labels = file2matrix('data/datingTestSet.txt')
    ...
    for i in range(num_test):
        classify_result = knn_classify(normalize_dating_data_matrix[i, :],normalize_dating_data_matrix[num_test: m, :],dating_labels[num_test:m], 7)
...
def classify_person():
    ...
    dating_data_matrix, dating_labels = file2matrix('data/datingTestSet2.txt')
    normalize_dating_data_matrix, ranges, min_val = normalize(dating_data_matrix)
